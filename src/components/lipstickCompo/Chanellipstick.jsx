...
class Chanellipstick extends Component {
  constructor(props) {
    super(props);
    this.state = {
      divcontainer: false,
  ...
  render() {
    ...
    return (
      <section className="foundation-container container">
        <h3 className="foundation-title capital">chanel</h3>
        <div className="foundation-product-container container">
          {shopproducts.map((item) =>
            item.id == 8
              ? item.lipstick_chanel
                  .filter((user) => user.id < 117)
                  .map((user) => (
                    <article className="product">
                      <Link to={`/detaillipchanel/${user.id}`}>
